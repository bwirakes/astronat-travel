...
/** One of the four geodetic angles. Layout-stable shape — the Place tab
 *  feeds real data; older call-sites fall back to the mock set below. */
export interface GeodeticGridAngle {
    name: string;          // "ASCENDANT", "NADIR", "DESCENDANT", "MIDHEAVEN"
    abrv: "ASC" | "IC" | "DSC" | "MH" | "MC";
    sign: string;          // "Sagittarius"
    deg: string;           // "10°" — already formatted
    desc: string;          // "Rising energy, personal projection…"
    hit: { p: string; type: string } | null;
}
...
const MOCK_ANGLES: GeodeticGridAngle[] = [
    { name: "ASCENDANT",  abrv: "ASC", sign: "Sagittarius", deg: "10°",
      desc: "Rising energy, personal projection, and the face you show the world here.",
      hit: { p: "Jupiter", type: "conjunct" } },
    { name: "NADIR",      abrv: "IC",  sign: "Pisces",      deg: "14°",
      desc: "Roots, home, and internal base. Represents physical anchoring.",
      hit: null },
    { name: "DESCENDANT", abrv: "DSC", sign: "Gemini",      deg: "10°",
      desc: "Partnerships and outward reflection. Who you attract in this location.",
      hit: { p: "Mercury", type: "trine" } },
    { name: "MIDHEAVEN",  abrv: "MH",  sign: "Virgo",       deg: "14°",
      desc: "Career, authority, and public legacy. How your work is received.",
      hit: { p: "Venus", type: "square" } },
];

export default function GeodeticGridCard({ destination = "This City", angles }: Props) {
    const rows = angles ?? MOCK_ANGLES;
    return (
        <div className="w-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {rows.map((a, i) => (
                    <div
                        key={`${a.abrv}-${i}`}
                        className="flex flex-col bg-[var(--surface)] border border-[var(--surface-border)] rounded-[var(--shape-asymmetric-md)] p-6 relative group transition-colors hover:border-[var(--text-tertiary)]"
                    >
