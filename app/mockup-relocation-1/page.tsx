...
// ═══════════════════════════════════════════════════════════════
// SVG COMPONENT (WITH GLOBAL SPIN)
// ═══════════════════════════════════════════════════════════════

function RelocatedWheelSVG({ isDark }: { isDark: boolean }) {
  const c = {
  ...
  return (
    <svg viewBox="0 0 800 800" xmlns="http://www.w3.org/2000/svg" style={{ overflow: "visible" }}>
      ...
      <g id="rotating-houses" opacity={isDark ? "0.55" : "0.9"} style={{ transformOrigin: "400px 400px" }}>
        {HOUSES.map((cusp, i) => {
      ...
      {/* STATIC CONTENT (Zodiac, Aspects, Planets) */}
      <g id="static-zodiac">
        
        {/* Zodiac boundaries & fills */}
        {SIGNS.map((s) => {
        ...
        {PLANETS.map((p) => {
          ...
          return (
            <g key={p.id} id={`planet-${p.id}`} opacity="0" style={{ transformOrigin: `${pt.x}px ${pt.y}px` }}>
              ...
              <text x={pt.x} y={pt.y} textAnchor="middle" dominantBaseline="central" fontSize="20" fill={p.color} fontFamily="var(--font-secondary)" filter={`url(#glow-${p.id})`}>
                {p.glyph}
              </text>
            </g>
          );
...
export default function RelocationGlobalSpinPage() {
  ...
  useGSAP(() => {
    ...
    if (introCard) {
      ...
      ScrollTrigger.create({
        ...
        start: "top 80%",
        onEnter: () => {
          gsap.to(introCard, { autoAlpha: 1, y: 0, duration: 0.8, ease: "power3.out" });
          
          // SPIN IT!
          gsap.to("#rotating-houses", {
            rotation: ROTATION_SHIFT,
            duration: 2.5,
            ease: "circ.inOut", // Nice dramatic ease
            delay: 0.5
          });
        },
        ...
        onLeaveBack: () => {
          gsap.to(introCard, { autoAlpha: 0, y: 30, duration: 0.4 });
          // Reverse spin to original!
          gsap.to("#rotating-houses", {
            rotation: 0,
            duration: 2.0,
            ease: "power2.inOut"
    ...
    PLANETS.forEach((p) => {
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          // Check if planet is already visible (so we don't re-animate it if it's staying in place)
          const planetEl = document.getElementById(`planet-${p.id}`);
          if (planetEl && planetEl.style.opacity !== "1") {
            gsap.to(`#planet-${p.id}`, { opacity: 1, scale: 1, duration: 0.9, ease: "back.out(1.7)" });
          }
          myAspects.forEach((a, i) => {
            gsap.to(`#aspect-${a.id}`, { opacity: 0.55, strokeDashoffset: 0, duration: 1.4, delay: i * 0.08 });
          });
        },
        onLeave: () => {
          // Hide card, but leave planet and aspect visible so they stay on screen
          gsap.to(card, { autoAlpha: 0, y: -30, duration: 0.4 });
        },
        onEnterBack: () => {
          gsap.to(card, { autoAlpha: 1, y: 0, duration: 0.6 });
          // Ensure it's fully visible (in case it wasn't)
        ...
        onLeaveBack: () => {
          gsap.to(card, { autoAlpha: 0, y: 30, duration: 0.4 });
          // Reverse animation when scrolling back UP out of section
          gsap.to(`#planet-${p.id}`, { opacity: 0, scale: 0, duration: 0.4 });
          myAspects.forEach((a) => {
    ...
    <div ref={wrapperRef} className="relative" style={{ backgroundColor: isDark ? "#000000" : "var(--color-eggshell)", transition: "background-color 0.6s ease" }}>
      <div className="fixed inset-0 z-0 flex items-center justify-center pointer-events-none">
        <div style={{ width: "min(100vh, 820px)", aspectRatio: "1" }}>
          <RelocatedWheelSVG isDark={isDark} />
        </div>
      </div>
      ...
      <div className="fixed top-0 left-0 w-full z-50 pointer-events-auto">
        <div style={{ background: isDark ? "linear-gradient(to bottom, rgba(0,0,0,0.8), rgba(0,0,0,0.4), transparent)" : "linear-gradient(to bottom, var(--color-eggshell), rgba(248,245,236,0.4), transparent)" }}>
          <Navbar activeHref="/mockup-relocation-1" centerContent={<span className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/40">Jakarta → London Shift</span>} />
        </div>
      </div>
          ...
          <div id="intro-card" className="max-w-2xl text-center">
            ...
            <div className="p-6 md:p-8 rounded-[var(--shape-asymmetric-lg)] backdrop-blur-xl max-w-xl mx-auto" style={{ background: isDark ? "rgba(0,0,0,0.60)" : "rgba(255,255,255,0.70)", border: "1px solid var(--surface-border)" }}>
              <p className="font-body text-base md:text-lg leading-relaxed shadow-sm" style={{ color: "var(--text-secondary)" }}>
                You were born in <strong style={{ color: "var(--text-primary)" }}>Jakarta</strong>, but the map of your potential shifts dramatically when you relocate. Watch as your natal wheel spins to align with <strong style={{ color: "var(--text-primary)" }}>London, UK</strong>.
              </p>
            </div>
          </div>
        </section>
